...
export function safeToSend(name: string, collection: Collection, dontTrowError: boolean = false): any | Error {
  let exists = false;
  let req: any = {};
  for (const request of collection.requests) {
    ...
    if (!request.host && !request.url && !dontTrowError)
      return error(
    ...
    if (!hostExists && !request.url && !dontTrowError)
